...
int input_read_parameters(
  ...
  /**MMmod MG*/
  /**reading mu and eta parameters*/

  class_read_int("use_MG",pba->use_MG);

  if (pba->use_MG > 0) {
     if (ppt->gauge == newtonian) {
        class_read_double("mg_z_init",pba->mg_z_init);
	if (pba->use_MG == 1) {
           class_read_double("mg_mu0",pba->mg_mu0);
           class_read_double("mg_eta0",pba->mg_eta0);
	} else if (pba->use_MG == 2) {
           class_read_double("mg_mu0",pba->mg_mu0);
           class_read_double("mg_eta0",pba->mg_eta0);
	   class_read_double("mg_lambda",pba->mg_lambda);
	   printf("READING %f \n",pba->mg_lambda);
	} else if (pba->use_MG == 3) {
	   class_read_double("linder_gamma",pba->gammaL);
	} else {
           printf("More parametrizations will come, be patient!");
	   return 0;
	}
     } else if (ppt->gauge == synchronous) {
        printf("MODIFIED GRAVITY ONLY WORKS IN NEWTONIAN GAUGE!!\n");
        return 0;  
     }
     printf("parameters: %f %f %f\n",pba->mg_z_init,pba->mg_mu0,pba->mg_eta0);
  }
...
printf("perturb_sampling_stepsize = %f \n",ppr->perturb_sampling_stepsize);
