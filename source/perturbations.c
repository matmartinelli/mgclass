    ...
    if (_scalars_) {
      ...
      if (ppt->has_cl_number_count == _TRUE_) {
        ...
        if (ppt->has_nc_lens == _TRUE_) {
          ...
          /** MMmog MG */
          if (pba->use_MG == 1){
             // Sources needed for MG evolution of phidot
             ppt->has_source_delta_m = _TRUE_;
             ppt->has_source_theta_m = _TRUE_;
          }
        }
...
int perturb_workspace_init(
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == newtonian) {
      ...
      //MMmod MG
      //define new indexes here

      class_define_index(ppw->index_mt_MGmu,_TRUE_,index_mt,1);          /**< mu(a,k) MG function */
      class_define_index(ppw->index_mt_MGmudot,_TRUE_,index_mt,1);       /**< \dot{mu}(a,k)  */
      class_define_index(ppw->index_mt_MGeta,_TRUE_,index_mt,1);         /**< eta(a,k) MG function */
      class_define_index(ppw->index_mt_MGetadot,_TRUE_,index_mt,1);      /**< \dot{eta}(a,k)  */
      class_define_index(ppw->index_mt_A_MG,_TRUE_,index_mt,1);
...
/**MMmod MG */
/** Function computing MG functions mu(a,k) and eta(a,k)
NEEDED: add more cases
NEEDED: add better explanation
*/
...
int modgrav(
  ...
  double a, H, aH, aHdot, Hdot, Ez, rho_m, omegaDE, omegaDEdot, omegaM, omegaMdot;
  ...
  FILE *f = fopen("MG_allz.txt", "a");
  FILE *f1 = fopen("MG_z01.txt", "a");
  FILE *f2 = fopen("MG_z1.txt", "a");
  FILE *f3 = fopen("MG_z3.txt", "a");

  // Set high-z cutoff properties
  // NEEDED: make sense of this!
  CUTOFF_ZMAX = 6.;
  CUTOFF_WIDTH = 5e-3;
  ...
  /**NEEDED: either extend or has to bomb out if not Lambda */
  ...
  if (pba->use_MG == 1) {
  //Planck late-time parametrization
  //NEEDED: change it to use whatever Omega_DE
     ppw->pvecmetric[ppw->index_mt_MGmu] = 1. + pba->mg_mu0 * omegaDE * pba->Omega0_lambda;
 
     ppw->pvecmetric[ppw->index_mt_MGmudot] = pba->mg_mu0 * omegaDEdot * pba->Omega0_lambda;

     ppw->pvecmetric[ppw->index_mt_MGeta] = 1. + pba->mg_eta0 * omegaDE * pba->Omega0_lambda;

     ppw->pvecmetric[ppw->index_mt_MGetadot] = pba->mg_eta0 * omegaDEdot * pba->Omega0_lambda;

  } else if (pba->use_MG == 2) {
  // Baker and Bull parametrization
  ...
  // cutoff tanh function at high redshift.
     ...
     //printf("TEST %f %f %f\n",pba->mg_lambda,aH,k);
     // mu, gamma
     ppw->pvecmetric[ppw->index_mt_MGmu]    = 1. + pba->mg_mu0  * omegaDE * gk * cutoff;
     ppw->pvecmetric[ppw->index_mt_MGeta] = 1. + pba->mg_eta0 * omegaDE * gk * cutoff;

     // Conformal time derivatives
     ppw->pvecmetric[ppw->index_mt_MGmudot] = (ppw->pvecmetric[ppw->index_mt_MGmu] - 1.) *
                                              (omegaDEdot/omegaDE + gkdot_over_gk + cutoffdot_over_cutoff);

     ppw->pvecmetric[ppw->index_mt_MGetadot] = (ppw->pvecmetric[ppw->index_mt_MGeta] - 1.) *
                                               (omegaDEdot/omegaDE + gkdot_over_gk + cutoffdot_over_cutoff);


  } else if (pba->use_MG == 3) {
  // gamma Linder parametrization
  // to be compared with MGCAMB
  // WARNING!!! Assumes flat LCDM Universe!!!

    omegaM    = (1. / Ez / Ez)*pow(a,-3.0)*(pba->Omega0_cdm+pba->Omega0_dcdmdr+pba->Omega0_b);//pba->Omega_M;// pvecback[pba->index_bg_Omega_m]
    omegaMdot = -omegaM*(3*aH+2*Hdot/H);




    ppw->pvecmetric[ppw->index_mt_MGmu]     = (2./3.)*pow(omegaM,pba->gammaL-1)*(pow(omegaM,pba->gammaL)+2.0-3.0*pba->gammaL+3.0*(pba->gammaL-0.5)*omegaM);
    ppw->pvecmetric[ppw->index_mt_MGeta]    = -1.0+2.0/ppw->pvecmetric[ppw->index_mt_MGmu];

     // Conformal time derivatives
    ppw->pvecmetric[ppw->index_mt_MGmudot]  = (pba->gammaL-1.0)*(omegaMdot/omegaM)*ppw->pvecmetric[ppw->index_mt_MGmu]+
                                              (2./3.)*pow(omegaM,pba->gammaL-1)*omegaMdot*(pba->gammaL*pow(omegaM,pba->gammaL-1)+3*(pba->gammaL-0.5));
    ppw->pvecmetric[ppw->index_mt_MGetadot] = -2.0*ppw->pvecmetric[ppw->index_mt_MGmudot]/pow(ppw->pvecmetric[ppw->index_mt_MGmu],2.0);
  }

  fprintf(f,"%f %f %f\n",-1+1/pvecback[pba->index_bg_a],ppw->pvecmetric[ppw->index_mt_MGmu],ppw->pvecmetric[ppw->index_mt_MGeta]);
  if (a < 1/1.09 && a>1/1.11) {
     fprintf(f1,"%f %f %f %f\n",-1+1/pvecback[pba->index_bg_a],k,ppw->pvecmetric[ppw->index_mt_MGmu],ppw->pvecmetric[ppw->index_mt_MGeta]);
  } else if (a < 1/1.99 && a>1/2.01) {
     fprintf(f2,"%f %f %f %f\n",-1+1/pvecback[pba->index_bg_a],k,ppw->pvecmetric[ppw->index_mt_MGmu],ppw->pvecmetric[ppw->index_mt_MGeta]);
  } else if (a < 1/2.99 && a>1/3.01) {
     fprintf(f3,"%f %f %f %f\n",-1+1/pvecback[pba->index_bg_a],k,ppw->pvecmetric[ppw->index_mt_MGmu],ppw->pvecmetric[ppw->index_mt_MGeta]);
  }

  ppw->pvecmetric[ppw->index_mt_A_MG] = 4.5 * rho_m * (a*a) * pvecmetric[ppw->index_mt_MGmu] * pvecmetric[ppw->index_mt_MGeta];
  fclose(f);
  fclose(f1);
  fclose(f2);
  fclose(f3);
...
//init vectors for MG
  ...
  ppw->pvecmetric[ppw->index_mt_MGmu] = 1.;

  ppw->pvecmetric[ppw->index_mt_MGmudot] = 0.;;

  ppw->pvecmetric[ppw->index_mt_MGeta] = 1.;

  ppw->pvecmetric[ppw->index_mt_MGetadot] = 0.;


  ppw->pvecmetric[ppw->index_mt_A_MG] = 0.;
...
//      FILE *f = fopen("file.txt", "w"); 
      //MMmod MG 
      if (pba->use_MG > 0) { 
         // Only switch on MG modifications after a certain redshift
         if (a > 1./(1. + pba->mg_z_init)) {

          // Get values of MG functions and other variables
          class_call(modgrav(pba, ppw->pvecback, ppw->pvecmetric, k, tau, ppw),
                     ppt->error_message,
                     ppt->error_message);

          phi = y[ppw->pv->index_pt_phi];
          aH = a * ppw->pvecback[pba->index_bg_H];
          Hdot = ppw->pvecback[pba->index_bg_H_prime];


          // Equation for psi (psi -> phi / gamma)
          ppw->pvecmetric[ppw->index_mt_psi] = (phi / ppw->pvecmetric[ppw->index_mt_MGeta])
                                           - 4.5 * (a2/k2) * ppw->rho_plus_p_shear;

          // Equation for phi' (using the derivative of the Poisson eqn.)
          ppw->pvecmetric[ppw->index_mt_phi_prime] =
               phi * (k2/ppw->pvecmetric[ppw->index_mt_A_MG]) * (ppw->pvecmetric[ppw->index_mt_MGetadot]/ppw->pvecmetric[ppw->index_mt_MGeta] 
            +  ppw->pvecmetric[ppw->index_mt_MGmudot]/ppw->pvecmetric[ppw->index_mt_MGmu] - aH)
            +  ppw->theta_m * ((1./3.) - a * Hdot / k2)
            -  ppw->pvecmetric[ppw->index_mt_psi] * aH;
          ppw->pvecmetric[ppw->index_mt_phi_prime] *= ppw->pvecmetric[ppw->index_mt_A_MG] / (k2 + ppw->pvecmetric[ppw->index_mt_A_MG]);

         } else {
          // Use the original CLASS versions
          // Equation for psi
          ppw->pvecmetric[ppw->index_mt_psi] = y[ppw->pv->index_pt_phi]
                                             - 4.5 * (a2/k2) * ppw->rho_plus_p_shear;

          // Equation for phi'
          // PJB: Comes from the Einstein equations, so can't be used for MG
          ppw->pvecmetric[ppw->index_mt_phi_prime] =
                                -a_prime_over_a * ppw->pvecmetric[ppw->index_mt_psi]
                                + 1.5 * (a2/k2) * ppw->rho_plus_p_theta;
         } // end MG redshift check
